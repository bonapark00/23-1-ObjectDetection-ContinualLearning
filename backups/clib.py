...
'''
Flow of training
-> new sample passes to online_step from dataset -> follow from 1)...3)

main: online_step
       # if sample from new class (not seen before) 
        # new class sample affects both memory and the model.
        0) self.add_new_class: extend the head of FC classifier
            memory.add_new_class: revise class dictionary
            self.update_schedule(reset only, depends on learning sched)

       1) self.update_memory
            self.samplewise_importance: check if memory is full, if full, determines which sample to remove
                memory.replace_sample: if memory full, replace it with sample, else just append sample to memory
    
       2) self.online_train: train with random batch from memory
            self.sample_wise_loss_update: calculate current loss of all data in memory
                memory.update_loss_history: calculate the difference of current, prev loss and update Hi
       
       3) self.update_schedule: update scheduler with adaptive learning rate
            self.adaptive_lr: update learning rate
'''
...
class CLIB(ER):
    ...
    def online_step(self, sample, sample_num, n_worker):
        '''
        Do: Main function operates first
        Parameters: new sample
        '''
        ...
        if self.num_updates >= 1:
            train_loss, train_acc = self.online_train([], self.batch_size, n_worker,
                                                      iterations=int(self.num_updates), stream_batch_size=0) #online_train안에 sample wise loss update
            self.report_training(sample_num, train_loss, train_acc) #print accuracy in prompt
    ...
    def online_train(self, sample, batch_size, n_worker, iterations=1, stream_batch_size=0):
        '''
        Do: train the model with random batch from memory. Then, update Hi (sample importance) of
            data in train batch.

            *stream_batch is not used in CLIB
        Parameters: sample
        '''
        ...
        #get num of all images from memory if images stored in memory < batch size
        ...
        for i in range(iterations):
            ...
            if len(self.memory) > 0 and batch_size - stream_batch_size > 0:
                memory_data = self.memory.get_batch(memory_batch_size) #get random images from memory
            ...
            #logit==output, loss==cross entropy loss
            #x is from random batch
    ...
    def add_new_class(self, class_name):
        '''
        Do: append the sample to exposed_class, add the head of FC classifier. (prev weights are remained)
            also, depend on learning schedule, it could reset update_schedule
        
        Parameters: class of sample
        '''
    ...
    def update_schedule(self, reset=False):
        '''
        Do: update scheduler. 
        Parameters: reset=True might passed when new sample appears to memory. 
        '''
    ...
    def samplewise_loss_update(self, ema_ratio=0.90, batchsize=512):
        '''
        Do: Calculate current loss from all data in Memory (not a training)
            Then, update Hi by passing the current loss to memory.udate_history
        
        Parameters: ema_ratio: determines the lambda in exponential moving average
        '''
        ...
        if self.imp_update_counter % self.imp_update_period == 0:
            if len(self.memory) > 0:
                self.model.eval()
                with torch.no_grad():
                    x = self.memory.device_img #images from memory
    ...
    def samplewise_importance_memory(self, sample):
        '''
        Do: if the memory is full, it finds the lowest important sample (lowest Hi) 
            considering class balance (picked from highest frequency).
            Then, replaces it with new sample through memory.replace_sample.

            if the memory is not full, just appends new sample through memory.replace_sample

            *memory.replace_sample is used in both replacing and simply appending data

        Parameters: new sample from Dataset
        '''

        #Case need to remove unimportant sample
    ...
    def adaptive_lr(self, period=10, min_iter=10, significance=0.05):
        '''
        Do: alternate learning rate. (Details in sec4.4)
        Parameters: ''
        '''

...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets, for_distillation=False):
    ...
    if for_distillation:
        ...
        for i in range(batch_size):
            ...
            box_regression = box_regressions[i].reshape(512, box_regressions[i].size(-1) // 4, 4) 

            box_loss = F.smooth_l1_loss(
            box_regression[sampled_pos_inds_subset, labels_pos],
            regression_targets[i][sampled_pos_inds_subset],
            beta=1 / 9,
            reduction='sum',
            )
            box_loss = box_loss/512
        ...
        box_regression_loss = box_loss / labels.numel()
...
class RoIHeads(nn.Module):
    ...
    def generate_distillation_proposals(self, proposals, cls_logits, reg_logits, soft_num=64):
        ...
        cls_logits_list = cls_logits.split(boxes_per_image, 0)
        reg_logits_list = reg_logits.split(boxes_per_image, 0)
    ...
    def forward(self,
        ...
        if self.training:
            if distill_mode:
                #distill_mode means that there already exists proposals (from teacher model)
                assert teacher_proposals is not None
                proposals_distill= teacher_proposals
                box_features_distill = self.box_roi_pool(features, proposals_distill, image_shapes)
                box_features_distill = self.box_head(box_features_distill)  
                class_logits_distill, box_regression_distill = self.box_predictor(box_features_distill)
                boxes_per_image = [boxes_in_image.shape[0] for boxes_in_image in proposals_distill]
                
                class_logits_pl = class_logits_distill.split(boxes_per_image, 0)
                box_regression_pl = box_regression_distill.split(boxes_per_image, 0)
            ...
            if self.generate_soft_proposals:
                selected_props, selected_cls_logits, selected_reg_logits = self.generate_distillation_proposals(proposals, class_logits, box_regression, soft_num=self.soft_num)
                proposals_logits['proposals'] = selected_props
                proposals_logits['class_logits'] = selected_cls_logits
                proposals_logits['box_regression'] = selected_reg_logits

...
if not args.is_eval:
    ...
    for i in range(4):
        data_set = SODADataset(path="./dataset/SSLAD-2D", task_id=task_seed_list[args.seed_num - 1][i],
                                    split="train")
        train_loader_list.append(torch.utils.data.DataLoader(data_set, batch_size=args.batch_size, collate_fn=collate_fn))

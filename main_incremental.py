...
def main():
    args = config.incremental_parser()
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')

    note_suffix = f"_{args.note}" if args.note else ""
    log_path = f"logs/{args.dataset}/incremental/{args.mode}/sd-{args.seed_num}{note_suffix}.log"
    ...
    # Show arguments
    logging.info("Incremental training after joint training")
    ...
    logging.info(f"Incremental training: {task_seed_list[int(args.seed_num) - 1][2:]}")
    logging.info(f"note: {args.note}")

    checkpoint_path = f"model_checkpoints/{args.dataset}/joint/seed_{args.seed_num}{note_suffix}/epoch_{args.ckpt_epoch}.pth"
    ...
    tensorboard_path = f"{args.dataset}_incremental_{args.mode}_sd-{args.seed_num}{note_suffix}"
    ...
    method.model.load_state_dict(torch.load(checkpoint_path))
    method.model.train()
    ...
    for i, task in enumerate(selected_seed[2:]):
        ...
        for data in tqdm(train_task[task], desc=f"{args.mode} - Seed {args.seed_num} Task {task + 1} ({i+1}/2) training"):
    ...
    save_path = os.path.join('outputs', args.dataset, "incremental", args.mode, f"sd-{args.seed_num}{note_suffix}")

...
class CLAD_DER(CLAD_ER):
    def __init__(self, criterion, device, train_transform, test_transform, n_classes, writer, **kwargs):
        ...
        # self.model = select_model(model_name=None, dataset="clad", num_classes=n_classes, for_distillation=True).to(self.device)
    ...
    # This is cleaned code
    def online_train(self, sample, batch_size, n_worker, iterations=1, stream_batch_size=1):
        """
            Trains the model on the given sample.
            Args:
                sample (dict): Sample from the stream consisting of image, objects, and image_id
                batch_size (int): Batch size for training
                n_worker (int): Number of worker, default zero
                iterations (int): Number of iterations to train on the sample
            Returns:
                total_loss (float): Total loss of the model
        """
        ...
        self.count_log += (self.temp_batchsize + memory_batch_size)
        ...
        for i in range(iterations):
            ...
            # Get data from CladStreamDataset
            if self.temp_batchsize > 0:
            ...
            if memory_batch_size > 0:
                ...
                # print(f'distill_cl: {(self.alpha * distill_cls/memory_batch_size)}   distill_rg :{self.beta * distill_reg.detach()/memory_batch_size}')
                #breakpoint()
        ...
        for item in ['proposals', 'class_logits', 'box_regression']:
            proposals_logits[item] = proposals_logits[item][:self.temp_batchsize] 

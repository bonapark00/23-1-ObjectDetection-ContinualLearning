...
class ER:
    ...
    def add_new_class(self, class_name):
        ...
        #change fc layer. output +1/new class has appeared
        self.model.fc = nn.Linear(self.model.fc.in_features, self.num_learned_class).to(self.device)

        with torch.no_grad():
            if self.num_learned_class > 1:
                #pass prev_weight for existed layers
                self.model.fc.weight[:self.num_learned_class - 1] = prev_weight
        
        #optimizer.param_groups[0], optimizer.param_groups[1] = fc param
        for param in self.optimizer.param_groups[1]['params']:
            if param in self.optimizer.state.keys(): #state
    ...
    def online_train(self, sample, batch_size, n_worker, iterations=1, stream_batch_size=1):
        ...
        for i in range(iterations):
            ...
            if len(self.memory) > 0 and batch_size - stream_batch_size > 0:
                #get_batch -> returns randomly chosen images with size (memory_batch_size) 

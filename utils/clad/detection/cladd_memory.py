...
class CladMemoryDataset(MemoryDataset):
      def __init__(self, dataset, transform=None, cls_list=None, device=None, test_transform=None,
        ...
        self.img_location_count = {'Citystreet': 0, 'Countryroad': 0, 'Rainy': 0}
        ...
        self.obj_cls_count = np.zeros(np.max(self.obj_cls_list)) #[2,0,0,4]  2 objects in cat1, 4 objects of cat 4, in total
        self.obj_cls_train_cnt = np.zeros(np.max(self.obj_cls_list)) #[1,0,0,2] 1 time for cat1 obj, 2 times for cat4 are trained
      ...
      def replace_sample(self, sample, idx=None):
            ...
            if self.data_dir is None:
                  img_path = os.path.join("dataset", self.dataset,sample['split'],img_name)
            else:
      ...
      def get_batch(self, batch_size, use_weight=False, transform=None):
            ...
            for obj in list(itemgetter(*indices)(self.objects)):

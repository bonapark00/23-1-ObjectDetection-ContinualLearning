...
class MemoryDataset(Dataset):
    ...
    def add_new_class(self, cls_list):
        self.cls_list = cls_list
        self.cls_count.append(0) #for new class
        self.cls_idx.append([])  #for new class
    ...
    #method that returns random images of batch_size with memory holding images
    @torch.no_grad()
    def get_batch(self, batch_size, use_weight=False, transform=None):
        ...
        if self.keep_history:
            self.previous_idx = np.append(self.previous_idx, indices) #total history of indices selected for batch
        return data
    ...
    def update_loss_history(self, loss, prev_loss, ema_ratio=0.90, dropped_idx=None):
        ...
        self.previous_idx = np.array([], dtype=int) #previous_idx => train에 쓰인 batch idx
